      ...
      if (isMaster == true)
      {
        if (BufferSize > 0)
        {
          if (strncmp((const char *)Buffer, (const char *)PongMsg, 4) == 0)
          {
            ...
            APP_LOG(TS_ON, VLEVEL_L, "LORA: %d; FSK: %d\n\r", USE_MODEM_LORA, USE_MODEM_FSK);

            APP_LOG(TS_ON, VLEVEL_L, "LORA SPREADING FACTOR: %d\n\r",LORA_SPREADING_FACTOR);

            APP_LOG(TS_ON, VLEVEL_L, "LORA CODING RATE: %d\n\r",LORA_CODINGRATE);
      ...
      else
      {
        if (BufferSize > 0)
        {
          if (strncmp((const char *)Buffer, (const char *)PingMsg, 4) == 0)
          {
            ...
            APP_LOG(TS_ON, VLEVEL_L, "Transfer Count: %s\n\r",readCount);
